...
public class ChatApp extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel8.setFont(new java.awt.Font("Times New Roman", 2, 30));
        jLabel8.setForeground(new java.awt.Color(0, 204, 204));
        jLabel8.setText("             Lets Chat");
                   ...
                   try {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    while(true) {   
                                        String msgg = in.readUTF();
                                        jTextArea1.setEditable(true);
                                        jTextArea1.append(person2+": "+msgg+"\n");
                                        jTextArea1.setEditable(false);
                                    }
                                } catch(SocketException ee) {
                                    JOptionPane.showMessageDialog(null,"You've Ended the Chat");
                                } catch(EOFException ee) {
                                    JOptionPane.showMessageDialog(null,person2+" Has Ended the Chat");
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    ...
                                    statusVerifier(false);
                                    JOptionPane.showMessageDialog(null,"You have Ended the Chat");
                                } catch(Exception ee) {
                                    JOptionPane.showMessageDialog(null,ee.toString());
                                }
                            }
    ...
    void statusVerifier(boolean status1) {
        if(status1==false && cstatus.equals("Disconnected")) {
            try {
                ...
                if (jRadioButton1.isSelected() && ssc != null) {
                    ssc.close();
                }
                if (sc != null) {
                    sc.close();
        ...
        else if(status1==true && cstatus.equals("Connected")) {
    ...
    void firsttimestatusverifier(boolean status2) {
        if(status2 == false && cstatus.equals("Disconnected")) {
            try {
                jButton4.setEnabled(false);
                jButton3.setEnabled(true);
                jRadioButton1.setEnabled(true);
